...
switch($this->active_tab) {
    ...
    case 'bibtex':
        ...
        if(isset($_GET['status'])) {
              switch (sanitize_text_field($_GET['status'])) {  // Sanitize input
                case 'success':
                    echo "<h3>" . __('The following items have been added to the database', 'latex2html') .":</h3><hr />";
                    global $wpdb;
                    $tab_name = $wpdb->prefix . 'l2hbibtex';
                    $bibkeys = explode(' ', sanitize_text_field($_GET['bibkeys']));  // Sanitize input
                    echo "<div class='bibtex'>\n<ol>";
                    foreach($bibkeys as $bibkey) {
                      $sql = $wpdb->prepare("SELECT * FROM " . $tab_name . " WHERE `bibkey` = %s LIMIT 1;", sanitize_text_field($bibkey));  // Use prepared statement
			$bibitem = $wpdb->get_results($sql, ARRAY_A);
			if (!empty($bibitem) && isset($bibitem[0])) {
			  // Check if $bibitem is not empty and if there is an element at index 0
			  //die( var_export($bibitem));
			  echo l2h_bibtex_class::l2h_bibtex_render($bibitem[0]);
			} 
                    }
                    echo "</ol></div>";
                    break;
                ...
                default:
                    ...
                    echo sprintf("<p>" . __("Please check your bibtex data, the key %s already exists in the database.", 'latex2html') . "</p>", "<strong>" . esc_html($_GET['message']) . "</strong>");  // Escape output
            }
        }
        echo "</div>";
        break;
